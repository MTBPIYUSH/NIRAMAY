...
import { mockComplaints, mockEcoProducts, mockLeaderboard } from '../data/mockData';
...
export const CitizenDashboard: React.FC<CitizenDashboardProps> = ({ user, onLogout }) => {
  ...
  const userRank = mockLeaderboard.find(entry => entry.id === user.id)?.rank || 4;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8">
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <div className="bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-2xl font-bold text-gray-800">🎁 Rewards & Points</h3>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border border-green-100">
                    <div className="flex items-center">
                      <Trophy size={24} className="text-green-600 mr-3" />
                      <div>
                        <p className="font-semibold text-gray-800">View Leaderboard</p>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between p-4 bg-gradient-to-r from-orange-50 to-red-50 rounded-2xl border border-orange-100 hover:shadow-lg transition-shadow"
                  >
                    <div className="flex items-center">
                      <Gift size={24} className="text-orange-600 mr-3" />
                      <div className="text-left">
                        <p className="font-semibold text-gray-800">Redeem Rewards</p>
                        <p className="text-sm text-gray-600">Shop eco-friendly products</p>
                      </div>
                    </div>
                  ...
                  {mockLeaderboard.slice(0, 5).map((entry, index) => (
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className={`font-semibold ${entry.id === user.id ? 'text-green-800' : 'text-gray-800'}`}>
                            {entry.name} {entry.id === user.id && '(You)'}
                          </p>
                          <p className="text-sm text-gray-600">{entry.reportsCount} reports</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-lg text-gray-800">{entry.points}</p>
            ...
            {/* Eco Marketplace Preview */}
            <div className="bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-gray-800 flex items-center">
                  <ShoppingBag className="text-green-600 mr-3" size={28} />
                  🛒 Eco Marketplace
                </h3>
                <button
                  onClick={() => setActiveTab('store')}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {mockEcoProducts.slice(0, 3).map(product => (
                  <div key={product.id} className="group bg-gradient-to-br from-gray-50 to-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                    <img
                      src={product.image}
                      alt={product.name}
